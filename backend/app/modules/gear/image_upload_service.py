    ...
    async def upload_image(self, file: UploadFile, item_id: str, user_id: str, is_primary: bool = False) -> dict:
        ...
        if HAS_MAGIC:
            try:
                ...
                logger.warning(f"Failed to detect MIME type with magic: {e}, falling back to Pillow")
                detected_mime = None
        ...
        if not detected_mime:
            try:
                ...
                logger.error(f"Failed to detect MIME type with Pillow: {e}")
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Failed to detect file type",
                )
        ...
        try:
            ...
            if stored_path:
                try:
                    ...
                    logger.info(f"Rolled back uploaded file: {stored_path}")
                except Exception as cleanup_error:
                    logger.error(f"Failed to cleanup uploaded file {stored_path}: {cleanup_error}")
