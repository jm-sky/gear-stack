#!/usr/bin/env python3
"""Test script to verify Unsplash API search works correctly."""
...
async def test_unsplash_scraping():
    """Test Unsplash API search with a sample query."""
    async with AsyncSessionLocal() as db:
        repo = ImageSearchEngineRepository(db)
        service = ImageSearchService(db)

        # Get Unsplash engine
        engine = await repo.get_by_name("Unsplash")
        if not engine:
            print("❌ Unsplash engine not found!")
            print("   Run: python scripts/add_unsplash_search_engine.py")
            return
        ...
        print(f"  - API Key: {'***' if engine.api_key and engine.api_key != 'YOUR_UNSPLASH_ACCESS_KEY_HERE' else 'NOT SET'}")
        print(f"  - Response Mapping: {engine.response_mapping}")
        print()

        # Check if API key is set
        if not engine.api_key or engine.api_key == "YOUR_UNSPLASH_ACCESS_KEY_HERE":
            print("⚠️  API key not set!")
            print("   To set API key:")
            print("   1. Get free API key from: https://unsplash.com/developers")
            print("   2. Register your app at: https://unsplash.com/oauth/applications")
            print("   3. Update in database:")
            print(f"      UPDATE image_search_engines SET api_key = 'YOUR_ACCESS_KEY' WHERE id = '{engine.id}'")
            print("   4. Or set UNSPLASH_API_KEY environment variable and re-run add script")
            return
...
if __name__ == "__main__":
    asyncio.run(test_unsplash_scraping())
