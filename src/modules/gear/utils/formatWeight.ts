 ...
 */
export function resolveAutoWeightUnit(weightInGrams: number, autoMode: 'auto-g-kg' | 'auto-oz-lb'): TGearWeightUnit {
 ...
 */
function formatNumberWithSeparator(value: number, locale: string = DEFAULT_LOCALE_ID): string {
  const formatter = new Intl.NumberFormat(locale, {
    minimumFractionDigits: 0,
    maximumFractionDigits: WEIGHT_DECIMAL_PLACES,
  })
...
export function formatWeight(weight: number, unit: TGearWeightUnit, locale?: string): string {
  const formattedValue = formatNumberWithSeparator(weight, locale)
...
export function formatWeightFromGrams(weightInGrams: number, locale?: string): string {
  if (weightInGrams >= GRAMS_PER_KILOGRAM) {
    ...
    const formattedValue = formatNumberWithSeparator(weightInKg, locale)
    return `${formattedValue} kg`
  }
  const formattedValue = formatNumberWithSeparator(weightInGrams, locale)
  return `${formattedValue} g`
}
...
export function formatWeightToPreferredUnit(weightInGrams: number, preferredUnit: TGearWeightUnit, locale?: string): string {
  ...
  switch (preferredUnit) {
    ...
    case 'lb': {
      ...
      const formattedValue = formatNumberWithSeparator(weightInLb, locale)
      return `${formattedValue} lb`
    }
    case 'oz': {
      const weightInOz = weightInGrams / GRAMS_PER_OUNCE
      const formattedValue = formatNumberWithSeparator(weightInOz, locale)
      return `${formattedValue} oz`
    }
    case 'g':
    default: {
